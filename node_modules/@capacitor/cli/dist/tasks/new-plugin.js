...
async function newPlugin(config) {
    ...
    if (answers.confirm) {
        ...
        await common_1.runTask('Installing NPM dependencies', async () => {
            await common_1.runCommand(`cd "${pluginPath}"`);
            return common_1.runCommand('npm install');
        });
...
async function createIosPlugin(config, pluginPath, domain, className, answers) {
    ...
    const originalPluginSwift = await fs_1.readFileAsync(path_1.join(newPluginPath, 'Plugin/Plugin.swift'), 'utf8');
    const originalPluginObjc = await fs_1.readFileAsync(path_1.join(newPluginPath, 'Plugin/Plugin.m'), 'utf8');
    ...
    if (!answers.git) {
        common_1.logWarn('You will need to add a hompage and git repo to your generated podspec before installing or CocoaPods will complain');
    }
    ...
    await fs_1.writeFileAsync(path_1.join(newPluginPath, `Plugin/Plugin.swift`), pluginSwift, 'utf8');
    await fs_1.writeFileAsync(path_1.join(newPluginPath, `Plugin/Plugin.m`), pluginObjc, 'utf8');
}
...
async function createAndroidPlugin(config, pluginPath, domain, className) {
    ...
    const newPluginPath = path_1.join(pluginPath, 'android/', pluginPath);
    // Move the 'plugin' folder inside $pluginPath/android/plugin to be the same name as the plugin
    const gradleProjectPath = path_1.join(pluginPath, 'android/plugin');
    await fs_extra_1.move(gradleProjectPath, newPluginPath);
