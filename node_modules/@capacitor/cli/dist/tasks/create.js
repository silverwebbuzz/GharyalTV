...
async function createCommand(config, dir, name, id) {
    try {
        ...
        if (typeof e === 'string') {
            common_1.log('Usage: npx @capacitor/cli create appDir appName appId');
...
async function installDeps(config, dir) {
    await common_1.runTask(chalk_1.default `Installing dependencies`, async () => {
        return common_1.runCommand(`cd "${dir}" && npm install --save @capacitor/cli @capacitor/core`);
    });
}
