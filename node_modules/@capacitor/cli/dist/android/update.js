...
async function installGradlePlugins(config, capacitorPlugins, cordovaPlugins) {
    ...
    const relativeCapcitorAndroidPath = path_1.relative(settingsPath, capacitorAndroidPath).replace(/\\/g, '/');
...
${capacitorPlugins.map(p => {
        const relativePluginPath = path_1.relative(settingsPath, p.rootPath).replace(/\\/g, '/');
        return `
...
`;
    ...
    cordovaPlugins.map(p => {
        const relativePluginPath = path_1.relative(dependencyPath, p.rootPath).replace(/\\/g, '/');
...
async function handleCordovaPluginsGradle(config, cordovaPlugins) {
    ...
    cordovaPlugins.map(p => {
        const relativePluginPath = path_1.relative(pluginsFolder, p.rootPath).replace(/\\/g, '/');
        const frameworks = plugin_1.getPlatformElement(p, platform, 'framework');
        frameworks.map((framework) => {
